...
const AUCTION_ID = process.argv
  ...
  ?.split('=')[1];
const MODE =
  process.argv.find((arg) => arg.startsWith('--mode='))?.split('=')[1] || 'war';
const SEED_FILE = process.argv
  .find((arg) => arg.startsWith('--useSeedFile='))
  ?.split('=')[1];
...
function triggerNextRaceRound() {
  const nextPrice =
    Math.max(currentHighestPrice, currentTargetPrice) + tickSize;
...
async function run() {
  console.log(
    `🚀 Starting Bot Simulator [Mode: ${MODE}] for Auction: ${AUCTION_ID}`,
  );
  ...
  for (let i = 0; i < numToSpawn; i++) {
    ...
    socket.on('auctionState', (auction: any) => {
      ...
      if (highest > currentHighestPrice) {
        ...
        if (i === 0) {
          console.log(
            `ℹ️ Initialized price from auction state: ₹${currentHighestPrice.toLocaleString()} (Tick size: ₹${tickSize.toLocaleString()})`,
          );
        }
      }
    });
    ...
    socket.on('bidError', (err) => {
      ...
      if (
        MODE === 'race' &&
        err.message === 'The price is already bid. Try the next highest bid.'
      ) {
        const failedPrice = botLastBidPrices[i];
        if (failedPrice) {
          errorCountsByPrice[failedPrice] =
            (errorCountsByPrice[failedPrice] || 0) + 1;
          if (
            errorCountsByPrice[failedPrice] >= 5 &&
            lastTriggeredPrice < failedPrice
          ) {
            lastTriggeredPrice = failedPrice;
            console.log(
              `⚠️ Received ${errorCountsByPrice[failedPrice]} error messages for ₹${failedPrice.toLocaleString()}. Triggering next bidding round...`,
            );
    ...
    socket.on('newBid', (data) => {
      ...
      if (i === 0) {
        console.log(
          `📢 Live Bid Update: ₹${amount.toLocaleString()} (Leader: ${data.bid.vendor?.name || data.bid.vendorId})`,
        );
      }
    });
  ...
  console.log(
    `✅ Auction state initialized. Starting simulation at ₹${currentHighestPrice.toLocaleString()}...`,
  );
